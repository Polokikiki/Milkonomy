export default {
  ...
  "基准=该专业当前等级下利润最高的N个项目（N=基准项目数）；提升量=换装后在N个项目上的平均提升；护符只算经验。建议装备后的 +N 为强化等级，成本为该强化等级的市场买价。性价比列=每100万金币的提升量。": "基準=該專業當前等級下利潤最高的N個項目（N=基準項目數）；提升量=換裝後在N個項目上的平均提升；護符只算經驗。建議裝備後的 +N 為強化等級，成本為該強化等級的市場買價。性價比列=每100萬金幣的提升量。",
