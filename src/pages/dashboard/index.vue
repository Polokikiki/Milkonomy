...
import { Delete, Edit, Search, Setting, Star, StarFilled, Warning } from "@element-plus/icons-vue"
...
<template>
  <div class="app-container">
    <div class="game-info">
      <GameInfo />
      <div>
        <ActionConfig :show-compare="false" />
      </div>
    ...
    <el-row :gutter="20" class="row">
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="16">
        <el-card>
          ...
          <template #default>
            <el-table :data="leaderboardData" v-loading="loadingLD" @sort-change="handleSortLD" style="overflow-x:auto">
              ...
              <template v-for="colKey in ldColumnOrder" :key="colKey">
                ...
                <el-table-column v-if="colKey === 'profitPD' && ldColumnVisible.profitPD" prop="result.profitPD" sortable="custom" :sort-orders="['descending', null]" :label="t('利润 / 天')" align="center" min-width="120">
                  <template #default="{ row }">
                    <span :class="row.hasManualPrice ? 'manual' : ''" style="word-break:break-all;display:inline-block;max-width:200px">{{ row.result.profitPDFormat }}</span>&nbsp;
                ...
                <el-table-column v-if="colKey === 'profitPH' && ldColumnVisible.profitPH" prop="result.profitPH" sortable="custom" :sort-orders="['descending', null]" :label="t('利润 / h')" align="center" min-width="120">
                  <template #default="{ row }">
                    <span style="word-break:break-all;display:inline-block;max-width:180px">{{ row.result.profitPHFormat }}</span>
                  </template>
                </el-table-column>
                <el-table-column v-if="colKey === 'profitRate' && ldColumnVisible.profitRate" prop="result.profitRate" :label="t('利润率')" min-width="120" align="center" sortable="custom" :sort-orders="['descending', null]">
                  <template #default="{ row }">
                    <span>{{ row.result.profitRateFormat }}</span>
                  </template>
                </el-table-column>
